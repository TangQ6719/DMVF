...
class Transformer(nn.Module):
    ...
    def forward(self, src, reg, tgt, src_mask, reg_mask, tgt_mask, memory_matrix):
        memory_gird, memory_reg = self.encode(src, src_mask, reg, reg_mask)
        return self.decode(memory_gird, memory_reg, src_mask, reg_mask, tgt, tgt_mask, memory_matrix=memory_matrix)

    def encode(self, src, src_mask, reg, reg_mask):
        return self.encoder(self.src_embed(src), src_mask, self.src_embed(reg), reg_mask)

    def decode(self, memory_gird, memory_reg, src_mask, reg_mask, tgt, tgt_mask, past=None, memory_matrix=None):
        ...
        return self.decoder(embeddings, memory_gird, memory_reg, src_mask, reg_mask, tgt_mask, past=past)


class Encoder(nn.Module):
    def __init__(self, layer_grid, N_grid, layer_reg, N_reg):
        super(Encoder, self).__init__()
        self.layers = clones(layer_grid, N_grid)
...
class Decoder(nn.Module):
    def __init__(self, layer, N):
        ...
        self.linear = nn.Linear(layer.size, layer.size)
    ...
    def forward(self, x, memory_gird, memory_reg, src_mask, reg_mask, tgt_mask, past=None):
        memory = self.featurefusion(memory_gird, memory_reg)
        # memory = memory_gird
        if past is not None:
            present = [[], []]
            x = x[:, -1:]
    ...
    def featurefusion(self, memory_grid, memory_reg):

        memory_reg = memory_reg.mean(-2, keepdim=True)
        memory_grid = self.linear(memory_grid)
        memory_reg = self.linear(memory_reg)
        x_cat = torch.cat([memory_grid, memory_reg.expand_as(memory_grid)], dim=-1)# b,1,512
#         x_cat = torch.cat([x, goabl_h_t.view(ng, -1, self.size).expand_as(x)], dim=-1)
        x_f = self.fuse_layer(x_cat) + memory_grid + memory_reg
...
class Base(AttModel):
    ...
    def make_model(self, tgt_vocab, cmn):
        ...
        model = Transformer(
            Encoder(EncoderLayer(self.d_model, c(attn), c(ff), self.dropout), self.n_enc_grid, EncoderLayer_reg(self.d_model, c(attn), c(ff), self.dropout), self.n_enc_reg),
    ...
    def __init__(self, args, tokenizer):
        ...
        self.n_enc_grid = args.n_enc_grid
        self.n_enc_reg = args.n_enc_reg
    ...
    def _forward(self, att_feats, reg_tensor, seq, att_masks=None, reg_masks=None):
        ...
        out = self.model(att_feats, reg_feats, seq, att_masks, reg_masks, seq_mask, memory_matrix=self.memory_matrix)
    ...
    def core(self, it, att_memory, reg_memory, state, mask, reg_mask):
        if len(state) == 0:
            ys = it.unsqueeze(1)
            past = [att_memory.new_zeros(self.num_layers * 2, att_memory.shape[0], 0, self.d_model),
                    att_memory.new_zeros(self.num_layers * 2, att_memory.shape[0], 0, self.d_model)]
        else:
            ys = torch.cat([state[0][0], it.unsqueeze(1)], dim=1)
            past = state[1:]
        out, past = self.model.decode(att_memory, reg_memory, mask, reg_mask, ys,
                                      subsequent_mask(ys.size(1)).to(att_memory.device), past=past,
