...
class AttModel(CaptionModel):
    ...
    #(it, pp_att_feats, p_att_masks, pp_reg_feats, p_reg_masks, state)
    def get_logprobs_state(self, it, att_feats, att_masks, reg_feats, reg_masks, state, output_logsoftmax=1):
        # 'it' contains a word index
        # xt = self.embed(it)

        output, state = self.core(it, att_feats, reg_feats, state, att_masks, reg_masks)
    ...
    def _sample_beam(self, att_feats, reg_tensor, att_masks=None, reg_masks=None, opt={}):
        fc_feats = att_feats
        ...
        pp_att_feats, p_att_masks, pp_reg_feats, p_reg_masks = self._prepare_feature(att_feats, att_masks, reg_tensor, reg_masks)

        assert beam_size <= self.vocab_size + 1, 'lets assume this for now, otherwise this corner case causes a few headaches down the road. can be dealt with in future if needed'
        seq = att_feats.new_full((batch_size * sample_n, self.seq_length), self.pad_idx, dtype=torch.long)
        ...
        logprobs, state = self.get_logprobs_state(it, pp_att_feats, p_att_masks, pp_reg_feats, p_reg_masks, state)

        pp_att_feats, p_att_masks, pp_reg_feats, p_reg_masks = utils.repeat_tensors(beam_size,
                                                                                  [
                                                                                   pp_att_feats, p_att_masks,pp_reg_feats, p_reg_masks]
                                                                                  )
        self.done_beams = self.beam_search(state, logprobs, pp_att_feats, p_att_masks, pp_reg_feats, p_reg_masks, opt=opt)
    ...
    def _sample(self, att_feats, reg_tensor, att_masks=None, reg_masks=None, opt={}):
        fc_feats = att_feats
        ...
        if beam_size > 1 and sample_method in ['greedy', 'beam_search']:
            return self._sample_beam(att_feats, reg_tensor, att_masks, reg_masks, opt)
        if group_size > 1:
            return self._diverse_sample(att_feats, att_masks, opt)
